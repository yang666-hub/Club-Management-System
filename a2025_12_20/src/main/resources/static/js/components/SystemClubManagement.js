...
window.SystemClubManagement = {
                    ...
                    <el-table :data="clubList" stripe>
                        ...
                        <el-table-column label="操作" width="200">
                            <template slot-scope="scope">
                                ...
                                <el-button type="primary" size="mini" @click="editClub(scope.row)">编辑</el-button>
                                <el-button type="danger" size="mini" @click="deleteClub(scope.row)">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
            ...
            <el-dialog :title="dialogTitle" :visible.sync="formDialogVisible" width="60%">
                <el-form :model="clubForm" :rules="clubRules" ref="clubForm" label-width="100px">
                    ...
                    <el-form-item label="管理员" prop="adminId">
                        <el-select v-model="clubForm.adminId" placeholder="请选择管理员" style="width: 100%" filterable>
                            <el-option 
                                v-for="user in clubAdmins" 
                                :key="user.id" 
    ...
    methods: {
        ...
        async fetchClubAdmins() {
            try {
                const result = await api.user.list({ userType: 2, size: 1000 });
                this.clubAdmins = result.records;
            } catch (error) {
                console.error('获取社团管理员失败:', error);
            }
        },
        async fetchClubs() {
            try {
